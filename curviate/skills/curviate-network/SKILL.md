...
| `5` | Tier not active, or the LinkedIn account lacks the subscription. On a non-premium account a nonexistent handle also returns this rather than `4`. | Verify the handle through `search people` before assuming a subscription is the fix. |
